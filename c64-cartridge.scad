...
module post() {
    ...
    union() {
        ...
        for(i=[0,1])
	    let (fillet_r = (i == 0? POST_FILLET_R : EDGE_R))
            rotate([0,180*i,0])
            translate([0,0,-POST_HEIGHT*i])
            difference() {
                cylinder(r=LEDGE_DIAMETER/2 + fillet_r, h=fillet_r);
                translate([0,0,fillet_r]) torus(LEDGE_DIAMETER/2, fillet_r);
            }
    }
            ...
            union() {
                difference() {
                    ...
                    translate([-CART_WIDTH/2, CONNECTOR_LENGTH, -CART_DEPTH/2])
                        cube([CART_WIDTH, /*WALL_THICKNESS*/1.6, CART_DEPTH]);
                }
